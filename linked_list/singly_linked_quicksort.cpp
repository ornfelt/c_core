...
/**
	Given a linked list, apply the Quick sort algorithm to sort the linked list. The
	important things about implementation are, that it changes pointers rather than
	swapping data.
    ...
    - Call partition function to get a pivot node placed at its correct position
		...
		then move it after the tail. If the node has a smaller value, then keep it at its
		current position. Return pivot node
