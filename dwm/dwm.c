...
// Focus nth monitor (called by view).
void
...
// Moves the selected client to a given tag and view.
void
tagview(const Arg *arg)
{
	...
	if ((arg->ui & 341) == 0  && selmon == mons) {
		...
		tagnewmon(arg);
		return;
		// If second mon and moving to odd tag
	} else if ((arg->ui & 341) > 0  && selmon != mons){
...
// User function to move a the selected client to a monitor in a given direction and view.
void
...
// Tag next monitor based on dirtomon.
void
...
// Move client to new monitor (called by tagview).
void
...
// Tag the nth monitor and view (called by tagview).
void
...
// Used for multiple monitors only
void
...
/**
  View (go to) tag. Will move to first monitor if odd tag (1,3,5,7,9) and second monitor if even tag (2,4,6,8).
  The commented part will go to previous tag when the current tag is selected again.
*/
void
view(const Arg *arg)
{
