...
retained and carried over for computation by the previous pointer while that of the next
node is after pointers similarly.

Now there is a memory-efficient version of Doubly Linked List that can be created using
only one space for the address field with every node. This memory efficient Doubly Linked
