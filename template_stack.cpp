...
/**
https://www.geeksforgeeks.org/implementing-stack-using-class-templates-in-cpp/?ref=rp

The task is to implement some important functions of stack like pop(), push(), display(),
topElement(), isEmpty(), isFull() using class template in C++. Stack is a linear data
...
same code for different data types. For example, a software company may need sort() for
different data types. Rather than writing and maintaining the multiple codes, we can write
...
See code below.
Here two data types (integer and string) are implemented using a single stack class.
First, two objects are taken one is for integer class and the second is for the string
class, Elements are inserted in both the classes using push() and isFull() method of stack
class. Elements are removed using pop and isEmpty() functions of stack class. Finally, the
top element is printed for each class using the top element() function.
*/
