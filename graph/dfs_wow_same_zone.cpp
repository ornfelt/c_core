...
public:
    ...
    void DFSUtilSameZone(int v, std::unordered_set<int>& visited, const int& target_v, std::unordered_map<int, int>& zones) {
        visited.insert(v);
        if (std::find(node_history[curr_start_id].begin(), node_history[curr_start_id].end(), v) == node_history[curr_start_id].end() && v != curr_start_id)
            node_history[curr_start_id].push_back(v);
        ...
        if (v == target_v) {
            ...
            if (should_print)
                std::cout << v << " ";
            for (const int& neighbour : graph[v]) {
                if (visited.find(neighbour) == visited.end() && zones[neighbour] == zones[target_v]) {
                    DFSUtil(neighbour, visited, target_v);
                }
            }
        }
    ...
    bool DFS_search_same_zone(int start_id, const int& target_id, std::unordered_map<int, int>& zones) {
        if (should_print)
            std::cout << "start_id: " << start_id << ", target_id: " << target_id << std::endl;
        curr_start_id = start_id;
        std::unordered_set<int> visited;
        ...
        target_found = false;
        for (int vertex : graph[start_id]) {
            if (visited.find(vertex) == visited.end() && !target_found && zones[vertex] == zones[start_id]) {
...
#if !OUTLAND && TRICKERER_SQL
            // Only search nodes from the same zone (will fail for northrend trickerer nodes)
            if (node_id != other_node_id && node_zones[node_id] == node_zones[other_node_id]) {
