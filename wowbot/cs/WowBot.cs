...
namespace WowBot
{
    internal class WowBot
    {
        ...
        void SetPlayerSettings()
        {
            ...
            try
            {
                ...
                connection = new MySqlConnection(
                    "Server=localhost;Database=acore_characters;User ID=acore;Password=acore;");
                connection.Open();
        ...
        // Start BOT
        public void StartBot(string bgInputArg, string factionInputArg)
        {
            if (!string.IsNullOrEmpty(bgInputArg))
                bgInput = bgInputArg;
            //if (!string.IsNullOrEmpty(factionInputArg))
            //factionInput = factionInputArg;
            //Console.WriteLine($"Args: {bgInput}, {factionInputArg}");
            //bool isAlly = factionInput.ToLower() == "ally";
        ...
        void StartBgBot(int bg, int bgTimer, bool isAlly, bool isLowLevel)
        {
            ...
            for (int i = 0; i < 100 && timeInBg < bgTimer; i++)
            {
                ...
                if (rand.Next(3) == 0)
                {
                    // Simulate mouse actions (you might need a different approach or library for this)
                    // ...
